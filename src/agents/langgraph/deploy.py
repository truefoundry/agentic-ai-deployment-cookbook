...
service = Service(
    name="test-langgraph-fastapi-server",
    image=Build(
        build_source=LocalSource(local_build=False),
        build_spec=DockerFileBuild(
            dockerfile_path="./Dockerfile_langgraph_agent_with_server",
            build_context_path="./",
        ),
    ),
    resources=Resources(
    ...
    ports=[
        Port(
            ...
            expose=True,
            app_protocol="http",
            host="test-langgraph-fastapi-server-sfhack-8000.ml.odsc-demo.truefoundry.cloud",
        )
    ],
    replicas=1.0,
)
