...
mcp = FastMCP(
    "research_report_generation", stateless_http=True
)  # stateless_http is required for StreamableHTTP communication
...
async def conduct_research_and_report(query: str) -> str:
    ...
    try:
        ...
        # run_agent() returns the FINAL report content as a string.
        report_content = await run_agent(query)
...
if __name__ == "__main__":
    ...
    print(
        f"Starting FastMCP Server '{mcp.name}' on http://{mcp.settings.host}:{mcp.settings.port}/mcp"
    )
